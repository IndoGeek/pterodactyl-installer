...
from .webserver import apply_ssl, enable_acme_webroot_site


def configure_ssl(web: str, panel_domain: str, wing_domain: str, email: str, php_version: str) -> None:
    ...
    # Certificates may pre-exist or have just been issued; in either case make sure the
    # web server actually serves them. Certbot's plugin edits live inside the site file
    # and a resumed run rewrites the panel site to plain HTTP, which leaves HTTPS broken
    # while the ssl checkpoint already says done. Re-apply the HTTPS server block
    # deterministically instead of relying on Certbot's in-place edits.
    if apply_ssl(web, panel_domain, php_version):
        say(f'HTTPS is enabled for {panel_domain}.', 'OK')