#!/usr/bin/env bash
#
# IndoGeek Pterodactyl Installer - global entry point.
# Installs any missing runtime dependency (Python 3), prepares the project's
# state/log directories, and runs the interactive installer.
#
# Usable from anywhere:
#   cd /some/dir && sudo /path/to/project/install.sh
#
set -euo pipefail
...
if [ "$(id -u)" -ne 0 ]; then
  if command -v sudo >/dev/null 2>&1; then
    echo "[SETUP] The installer must run as root; re-running through sudo..."
...
echo "[SETUP] Starting IndoGeek Pterodactyl installer..."
